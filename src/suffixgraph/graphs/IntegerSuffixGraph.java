 ...
 *
 */
public class IntegerSuffixGraph extends AbstractSuffixGraph<Node> {
...
/***** DEFINITIONS *****/  
   ...
   */
  public IntegerSuffixGraph(FastaSequence sequence, String path) {
   ...
   */
  public IntegerSuffixGraph(FastaSequence sequence, int length, boolean printStats, GraphvizGraph g) {
    this.sequence = sequence;
    this.er = new IntegerNode().getEdgeRuler();
    ...
    if (printStats) {
      if (g==null) nodes = MeasuredGraphOperations.buildSuffixGraph(sequence, er, length);
      else nodes = MeasuredGraphOperations.buildSuffixGraph(sequence, er, length, g);
    }
    else {
