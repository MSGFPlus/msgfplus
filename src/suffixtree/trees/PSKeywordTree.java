...
import suffixtree.matches.Coor;
...
public class PSKeywordTree {
   ...
   * @param aaCount the minimum number of amino acids to match for considering a partial match
   */
  public PSKeywordTree(ArrayList<ArrayList<Integer>> queries, MassSequence database, int aaCount) {
   ...
   */
  public ArrayList<MegaPrefixSuffixMatchObject> collectPrefixSuffixMatches() {
  	this.collectPM = new ProgressMeter("Collecting", this.queries.length, System.out);
  	this.collected = 0;
    ArrayList<MegaPrefixSuffixMatchObject> matches = new ArrayList<MegaPrefixSuffixMatchObject>();
    collectPrefixSuffixMatches(this.fRoot, new TreeMap<Integer,Integer>(), 0, new TreeMap<Integer,ArrayList<Coor>>(), matches);
    return matches;
  }
  ...
  private void collectPrefixSuffixMatches(ComplexInternalNode node, 
                                          ...
                                          ArrayList<MegaPrefixSuffixMatchObject> matches) {
    ...
    for (int i=0; i<node.getPrefixMatchCount(); i++) {
      ...
      for (long currentStart=start; currentStart<end; currentStart++) {
        ...
        if (currentStart-start < this.minAACount) continue;
    ...
    if (node.getPositions().length>0) {
      ...
      for (int queryIndex : node.getPositions()) {
        ...
        TreeMap<Integer,ArrayList<Coor>> suffixes = new TreeMap<Integer,ArrayList<Coor>>();
        while (true) {
          
          //System.out.printf("Collecting %d suffixes\n", currentNode.getPrefixMatchCount());
          //int matchesCount = 0;
          ...
          for (int i=0; i<currentNode.getPrefixMatchCount(); i++) {
            ...
            for (long currentEnd=end-1; currentEnd>=0; currentEnd--) {
              ...
              if (end-currentEnd < this.minAACount) continue;
              
              // finally add check whether there is a prefix match
              //int targetMass = parentMass - cumMass;
              if (!suffixes.containsKey(cumMass)) suffixes.put(cumMass, new ArrayList<Coor>());
              suffixes.get(cumMass).add(new Coor(currentEnd, end));
              
              /* 
              if (prefixes.containsKey(targetMass)) {
                
                System.out.printf("Target mass of %d has %d matches\n", targetMass, prefixes.get(targetMass).size());
                
                for (Coor c : prefixes.get(targetMass)) {
                  PrefixSuffixMatchObject m = new PrefixSuffixMatchObject(c.getStart(), 
                                                                          c.getEnd(), 
                                                                          currentEnd, 
                                                                          end,
                                                                          this.db,
                                                                          ...
                                                                          queryIndex);
                  matchesCount++;
                  long relStart = m.getRelativeStart();
                  long relEnd = m.getRelativeEnd();
                  if (relEnd - relStart < 800 && relEnd - relStart > 30) {
                    ...
                    if (!matches.containsKey(m)) {
                      ...
                      matches.get(m).addMiddle(c.getEnd(), currentEnd);
                    }
            ...
            // try to extend pass the last edge
            //for (long currentEnd; currentEnd >= 0;) {
        ...
        for (int prefixMass : prefixes.keySet()) {
          int targetMass = parentMass - prefixMass;
          if (suffixes.containsKey(targetMass)) {
            MegaPrefixSuffixMatchObject m = new MegaPrefixSuffixMatchObject(new ArrayList<Coor>(prefixes.get(prefixMass)), 
                                                                            suffixes.get(targetMass), 
                                                                            this.db, 
                                                                            query.getLabels(), 
                                                                            queryIndex);
            matches.add(m);
        ...
        //for (int key : suffixes.keySet()) {  System.out.println("Mass " + key);}
        System.out.printf("Prefixes %d Suffixes %d\n", prefixes.size(), suffixes.size());
        System.out.println("Added MegaPrefixSuffixMatches " + addedMatches);
      }
    }
    ...
    for (int i=0; i<node.getDegree(); i++) {
    	...
    	for (Coor coor : coors) {
    		int cumMass = 0;
    		for (long currentStart=coor.getEnd(); currentStart<this.db.getSize(); currentStart++) {
    			...
    			if (!prefixes.containsKey(shiftedMass)) prefixes.put(shiftedMass, new ArrayList<Coor>());
    			prefixes.get(shiftedMass).add(new Coor(coor.getStart(), currentStart+1));
    		}
    	}
    	...
    	// recurse
      collectPrefixSuffixMatches(nextNode, masses, cumTotalMass, prefixes, matches);
    ...
    for (long start=this.db.getSize()-1; start>=0; start--) {
      ...
      for (long i=start; i>=0 && this.db.hasMass(i); i--) {
        ...
        if (matchIndex>=0) {
          ...
          compCount += this.storeMatchesReverse(start, i-1, this.rRoot.getEdgeAt(matchIndex), 1);
        }
      }
  ...
  private void populateForward() {
    ...
    for (long start=0; start<this.db.getSize(); start++) {
      ...
      for (long i=start; i<this.db.getSize() && this.db.hasMass(i); i++) {
        ...
        if (matchIndex>=0) {
        	divider = matchIndex+1;
          compCount += this.storeMatchesForward(start, i+1, this.fRoot.getEdgeAt(matchIndex), 1);
        }
      }
   ...
   */
  private int storeMatchesReverse(long dbStart, long dbIndex, Edge e, int eIndex) {
    int compCount = 0;
      ...
      if (eIndex==e.size()) {
        ...
        for (long i=dbIndex; i>=0 && this.db.hasMass(i); i--) {
          ...
          if (matchIndex>=0) {
          	divider = matchIndex + 1;
            // recurse
            compCount += storeMatchesReverse(dbStart, i-1, sink.getEdgeAt(matchIndex), 1);
            hasMatch = true;
          }
        }
        lastNode = (ComplexInternalNode)sink;
        ...
        for (long i=dbIndex; i>=0 && this.db.hasMass(i); i--) {
          ...
          // There is a match
          compCount += storeMatchesReverse(dbStart, i-1, e, eIndex+1);
          hasMatch = true;
        }
    ...
    int extend = (int)(dbStart-dbIndex);
    //if (!hasMatch && matchedMass >= this.minPartialMatchMass) {
    if (!hasMatch && extend >= this.minAACount) {
      //System.out.printf("Extend %d from %d to %d, %s\n", extend, dbIndex, dbStart, this.db.getSubsequence(dbIndex+1, dbStart+1));
      lastNode.addPartialMatch(dbIndex+1, extend);
      this.partialMatches++;
    }
    return compCount;
  }
   ...
   * @return the number of comparisons done for this query.
   */
  private int storeMatchesForward(long dbStart, long dbIndex, Edge e, int eIndex) {
    int compCount = 0;
      ...
      if (eIndex==e.size()) {
        ...
        for (long i=dbIndex; i<db.getSize() && this.db.hasMass(i); i++) {
          ...
          if (matchIndex>=0) {
          	divider = matchIndex + 1;
            // recurse
            compCount += storeMatchesForward(dbStart, i+1, sink.getEdgeAt(matchIndex), 1);
            hasMatch = true;
          }
        }
        lastNode = (ComplexInternalNode)sink;
        ...
        for (long i=dbIndex; i<this.db.getSize() && this.db.hasMass(i); i++) {
          ...
          // There is a match
          compCount += storeMatchesForward(dbStart, i+1, e, eIndex+1);
          hasMatch = true;
        }
        lastNode = ((ComplexInternalNode)e.getSink());
      }
